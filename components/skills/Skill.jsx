        ...
        <>
            ...
            <SimpleGrid w={"full"} h={"full"} columns={[1,1,2, 2,3]} rounded={"xl"} mx={"auto"} p={[5, 10]} mt={5} spacingX={5} spacingY={10} id={"my-projects"}>
                ...
                {projects.map((project) => (
                    ...
                    <Box key={project.id} rounded={"2xl"}  mt={10} p={5} boxShadow={"2xl"} h={"100%"} minH={"100%"}>
                        <Link href={`${project.href}`} >
                            <ImageComp image={project.image} />

                        </Link>
                        <Text color={"#000"} textAlign={"center"} rounded={"xl"} pt={2} fontWeight={"500"} fontSize={[20, 25]}>{project.title}</Text>
                        <Divider />
                        <Text color={"#000"} textAlign={"center"} rounded={"xl"} pt={2} fontSize={[14, 16]}>{project.discription}</Text>
                    </Box>
